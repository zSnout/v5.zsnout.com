 ...
 */
async function drawOneBurningShip(size: number = canvasSize) {
...
/**
 * Draws the Burning Ship.
 * @returns A promise that resolves to a boolean indicating whether all fractals were successfully drawn.
 */
async function drawBurningShip() {
  let success = await drawOneBurningShip(340);
  if (success && canvasSize > 340) success = await drawOneBurningShip(680);
  if (success && canvasSize > 680) success = await drawOneBurningShip(1360);
  if (success && canvasSize > 1360) success = await drawOneBurningShip(2720);
  return success;
}

/**
 * Zooms in on a point in the Burning Ship with 2x zoom.
...
function setPageTitle() {
  document.title = `Burning Ship (${canvasSize / 680}x Resolution)`;
}
